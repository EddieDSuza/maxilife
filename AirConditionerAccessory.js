...
class AirConditionerAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        const characteristicCurrentTemperature = service.getCharacteristic(Characteristic.CurrentTemperature)
            .updateValue(this._getDividedState(dps[this.dpSwingMode], this.temperatureDivisor)) 
            .on('get', this.getDividedState.bind(this, this.dpSwingMode, this.temperatureDivisor));       
        ...
        if (!this.device.context.noHeat) {
            characteristicHeatingThresholdTemperature = service.getCharacteristic(Characteristic.HeatingThresholdTemperature)
                ...
                .updateValue(this._getDividedState(dps[this.dpThreshold], this.thresholdTemperatureDivisor))            
                .on('get', this.getDividedState.bind(this, this.dpThreshold, this.thresholdTemperatureDivisor))                
            ...
            if (changes.hasOwnProperty(this.dpActive)) {
                ...
                if (characteristicActive.value !== newActive) {
                    ...
                    if (!changes.hasOwnProperty(this.dpMode)) {
                        characteristicCurrentHeaterCoolerState.updateValue(changes[this._getCurrentHeaterCoolerState * this.targetTemperatureDivisor]);                        
                    }
    ...
    setTargetThresholdTemperature(mode, value, callback) {
        this.setState(this.dpThreshold, value * this.thresholdTemperatureDivisor, err => {
            if (err) return callback(err);
